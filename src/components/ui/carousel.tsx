...
function CarouselProvider({
  ...
  useEffect(() => {
    if (itemsCount > 1) {
      ...
      }, 3000); // Adjust interval time (3000ms = 3 seconds)

      return () => clearInterval(intervalRef.current as NodeJS.Timeout);
    }
  }, [itemsCount]);

  // Function to reset autoplay when the user interacts
  const resetAutoplay = () => {
...
function CarouselItem({ children, className }: CarouselItemProps) {
  return (
    <motion.div
      className={cn(
        'w-full min-w-0 shrink-0 grow-0 overflow-hidden border-none !border-0',
        className
      )}
    >
      {children}
